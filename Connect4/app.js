document.addEventListener('DOMContentLoaded', () => {
    // squares correspond à l'ensemble des div de la grille
    ...
    for (var i = 0, len = squares.length; i < len; i++) {
        (function (index) {
            // annonce d'1 onclick sur chaque carré de la grille
        ...
        // check the board for a win or loose
        function checkBoard() {
            // make const that shows all winnin Arrays
            const winningArrays = [
              ...
              for (let y = 0; y < winningArrays.length; y++) {
                  ...
                  // now check those arrays to see if they all have the class name of player-one
                  if (  square1.classList.contains('player-one') &&
                        square2.classList.contains('player-one') &&
                        square3.classList.contains('player-one') &&
                        square4.classList.contains('player-one')) {
                      // if they do, player-one passed as the winner
                      result.innerHTML = 'Player 1 Wins !';
                  }
                  // now check those arrays to see if they all have the class name of player-two
                  else if ( square1.classList.contains('player-two') &&
                            square2.classList.contains('player-two') &&
                            square3.classList.contains('player-two') &&
                            square4.classList.contains('player-two'))  {
                      // if they do, player-two passed as the winner
                      result.innerHTML = 'Player 2 Wins !';
                    }
              }
        }

        // add an eventListener to each square that will trigerre the checkBoard function on click
